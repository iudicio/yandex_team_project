...
В PR `iteration_2_review → main` входит только функционал Epic 1–5: поиск, экран команды, детали, избранное, фильтры по отрасли/зарплате и месту работы. Epic 0 уже принят в `main` через первую итерацию и не является изменением этого PR. Реализация и формальная проверка ведутся по [декомпозиции](./documentation/05-декомпозиция.md).

## MVP

- поиск вакансий с debounce и пагинацией;
- фильтры по отрасли, зарплате и месту работы;
...
Фильтрация по стране и региону включена в Epic 5 и в scope второй review-итерации.
...
| `iteration_2_review` | Только diff Epic 1–5: поиск, команда, детали, избранное и все фильтры. |

Описания формальных PR находятся в документах [первой](./documentation/09-iteration-1-review.md) и [второй](./documentation/10-iteration-2-review.md) итераций. PR второй итерации строится от актуального `main`, поэтому Epic 0 есть в итоговом дереве, но не в diff PR #10.
