...
В `main` находятся принятые Epic 0–3. Review-ветка `iteration_3_review` добавляет только Epic 4 «Отрасли и ЗП» и Epic 5 «Место работы». Состав сдачи, критерии и незаполненные фактические данные зафиксированы в [описании третьей итерации](./documentation/11-iteration-3-review.md).

## MVP

- поиск вакансий с debounce и пагинацией;
- фильтры по отрасли, зарплате, стране и региону;
...
| Роль | Epic 0–5 |
|---|---:|
| UI / дизайнер-разработчик | 19 задач, 68 SP |
| BE-1 / поиск и фильтры | 19 задач, 66 SP |
| BE-2 / детали, избранное и интеграция географии | 17 задач, 66 SP |
| **Итого** | **55 задач, 200 SP** |
...
| `iteration_3_review` | Только Epic 4–5: отрасль/зарплата и место работы; основа Epic 0–3 уже в `main`. |

Описания формальных PR находятся в документах [первой](./documentation/09-iteration-1-review.md), [второй](./documentation/10-iteration-2-review.md) и [третьей](./documentation/11-iteration-3-review.md) итераций. GitHub-вкладка Files changed для PR третьей итерации должна показывать Epic 4–5, но не повторять Epic 0–3.
...
- [Описание третьей итерации ревью](./documentation/11-iteration-3-review.md)
