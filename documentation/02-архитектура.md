...
| `Area` | Каталог стран и регионов | `id`, `name`, `parentId?`, `areas`. |
| `FilterSettings` | Сохранённые фильтры | `industry?`, `salary?`, `onlyWithSalary`, `country?`, `region?`; в query передаётся итоговый `area?`. |
