...
Для учебной приёмки используется View-based Android-приложение:

- Kotlin;
- XML layouts + ViewBinding;
- Single Activity + Fragments;
- Jetpack Navigation Component + BottomNavigationView;
- MVVM: Fragment → ViewModel → Interactor → Repository;
- LiveData для состояния экрана;
- Coroutines для асинхронной работы;
- Retrofit/OkHttp, Room, SharedPreferences, Glide, Koin.

Compose намеренно не выбирается: вводный материал предлагает сравнить подходы, но актуальные критерии явно проверяют ViewBinding, LiveData и BottomNavigationView.

## 2. Слои и зависимости

```mermaid
flowchart LR
    UI[UI<br/>Fragments, adapters, XML] --> P[Presentation<br/>ViewModels, UI state]
...
Toast, навигация и запуск системного chooser — одноразовые события. Они не должны повторяться после пересоздания Fragment. Конкретный механизм (`Event`, Channel/Flow bridge и т.п.) фиксируется в Epic 0 и используется одинаково.
...
- Fragment/ViewBinding не размещаются в DI.
...
HTTP-коды не должны анализироваться во Fragment. Текст ошибки выбирает UI через строковые ресурсы.
...
- В GitHub Actions используется secret `GH_API_ACCESS_TOKEN`.
